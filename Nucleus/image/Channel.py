...
def extract_channel(img_array, channel=2, gray=True):
    ...
    channel(int) -- int to represent which channel should be extracted;
    use the provided module variables (default:RED)
    gray (bool) -- If True, the channel image will be converted to grayscale,
    otherwise the resulting image is a color image with all other channels set
    to 0
...
def get_minmax(img_array, percent=0, channel=RED,
    ...
    Keyword arguments:
    img_array(ndarray) -- The color image to extract the channel from
    percent (float, optional) -- Determines how many pixels (in %) are ignored
    at either side of the histogram (useful to account for outliers; default:0)
    channel(int, optional) -- int to represent which channel should be
    extracted; use the provided module variables (default:RED)
    channel_only(bool) -- If True, it will be assumed that img_array only
    contains one channel (default:False)
    gray (bool, optional) -- If True, the channel image will be converted to
    grayscale, otherwise the resulting image is a color image with all other
    channels set to 0 (default:True)
...
def get_dynamic_range(img_array, channel=RED, channel_only=False,
    ...
    grayscale, otherwise the resulting image is a color image with all other
    channels set to 0 (default:True)
    in_percent(bool, optional) -- If true, the calculated dynamic range will be
    converted to float (between 0-1)

    Returns:
    int: If in_percent=False - The dynamic range of the respective channel
    (0-255)
...
def get_channel_histogram(img_array, channel=RED, channel_only=False,
    ...
    channel_only(bool) -- If True, it will be assumed that img_array only
    contains one channel (default:False)
    nbins(int, optional) -- The number of bins used to calculate the histogram
    (default:255)
...
def percentile_threshold(img_array, channel=RED,
    ...
    channel_only(bool) -- If True, it will be assumed that img_array only
    contains one channel (default:False)
    p0(float, optional) -- The percentile to calculate the threshold.
    (0-1;default:0.8)
    ignore(float, optional) -- The percentage of pixels to ignore in the
    calculation. Is used to account for outliers. (0-1;default:0.0005)
